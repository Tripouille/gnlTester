...
$(1MANDATORY): 1%: mandatory_start $(1MANDATORY_OBJS)
	clang++ -D BUFFER_SIZE=1 $(CPPFLAGS) $(UTILS) $(TESTS_PATH)$*.cpp $(MANDATORY_OBJS) && valgrind -q --leak-check=full --show-reachable=yes ./a.out && rm -f a.out
...
$(42MANDATORY): 42%: mandatory_start $(42MANDATORY_OBJS)
	@clang++ -D BUFFER_SIZE=42 $(CPPFLAGS) $(UTILS) $(TESTS_PATH)$*.cpp $(MANDATORY_OBJS) && valgrind -q --leak-check=full --show-reachable=yes ./a.out && rm -f a.out
...
$(10000000MANDATORY): 10000000%: mandatory_start $(10000000MANDATORY_OBJS)
	@clang++ -D BUFFER_SIZE=10000000 $(CPPFLAGS) $(UTILS) $(TESTS_PATH)$*.cpp $(MANDATORY_OBJS) && valgrind -q --leak-check=full --show-reachable=yes ./a.out && rm -f a.out
